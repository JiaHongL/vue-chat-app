...
<template>
  ...
  <!-- viewService.goToChatView() -->
	<div class="cursor-pointer mb-2" @click="setCurrentRoom('general')">
      <!-- 'bg-blue-100': chat.currentChatPartner()?.username === 'general' && !viewService.isMobile(),
      'bg-white': chat.currentChatPartner()?.username !== 'general' || viewService.isMobile(), -->
			<div 
				class="flex items-center p-2 rounded-lg shadow relative"
        :class="{
          'bg-blue-100': currentChatPartner?.username === 'general',
          'bg-white': currentChatPartner?.username !== 'general',
        }"
			>
	...
	<ul class="pb-[250px] sm:pb-0 sm:max-h-73 sm:overflow-scroll custom-scrollbar">
      ...
      >
        ...
        >
          <!-- (click)="store.setCurrentRoom(user.room);viewService.goToChatView()" -->
					<li 
            class="cursor-pointer mb-2"
            @click="setCurrentRoom(user.room)"
          >
              <!-- 'bg-blue-100': store.currentChatPartner()?.username === user.username && !viewService.isMobile(),
							...
							<div 
								...
								:class="{ 
									'bg-blue-100': currentChatPartner?.username === user.username,
									'bg-white': currentChatPartner?.username !== user.username,
								}"
							>
									<div class="relative flex-shrink-0 flex-grow-0">
